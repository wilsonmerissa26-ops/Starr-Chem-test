...
var block=/\n if\(\$\('check'\)\)\$\('check'\)\.onclick=function\(\)\{[\s\S]*?\n if\(\$\('chemTeach'\)\)/;
if(!block.test(s))throw new Error('missing disposable check handler block');
s=s.replace(block,"\n if($('chemTeach'))");
...
if(t.indexOf('math Check answer uses stable delegated capture handler')<0)t=replaceOnce(t,marker,extra,'delegated check regressions');
