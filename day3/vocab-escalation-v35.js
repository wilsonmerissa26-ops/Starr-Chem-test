...
function failedExplanationStreak(s,id){var a=attempts(s,id),n=0;for(var i=a.length-1;i>=0;i--){if(a[i].pass)break;if(['cold','review','support'].indexOf(a[i].kind)>=0)n++;}return n;}
...
function markTeaching(s,id){if(s.escalationTaughtTerm!==id){s.escalationTaughtTerm=id;s.escalationOriginReview=!!s.returnReview;s.escalationPostTeachRepair=false;}}
...
function install(doc){if(!doc||!doc.defaultView)return;var win=doc.defaultView,view=doc.getElementById('view');if(!view)return;function enhance(){var root=view.querySelector('[data-vocab-production-v3]');if(!root)return;var ph=phase(root),id=termIdFromTitle(title(root));if(!id)return;var s=read(win);if(!s)return;
...
return{MAX_FAILED_EXPLANATIONS:MAX_FAILED_EXPLANATIONS,TEACH_USE:TEACH_USE,termIdFromTitle:termIdFromTitle,failedExplanationStreak:failedExplanationStreak,shouldAutoTeach:shouldAutoTeach,markTeaching:markTeaching,clearEscalation:clearEscalation,install:install};
});