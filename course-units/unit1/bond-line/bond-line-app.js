(function () {
  ...
  function butaneImpliedHydrogensSvg(showGhosts) {
    ...
    butaneZigzagHydrogens.forEach(function (h, index) {
      var tx = h[0] + (h[0] < 100 ? -14 : h[0] > 590 ? 6 : -10);
      var ty = h[1] + (h[1] < 150 ? -3 : h[1] > 230 ? 18 : 7);
  ...
  function step5BondOverlays() {
    var svg = "";
    butaneCarbonSkeletonGeometry.bonds.forEach(function (bond, index) {
      svg += '<g data-step5-bond="BOND_' + (index + 1) + '" role="button" tabindex="0" aria-label="Bond segment ' + (index + 1) + ', not a carbon"><line x1="' + bond[0] + '" y1="' + bond[1] + '" x2="' + bond[2] + '" y2="' + bond[3] + '" style="stroke:transparent;stroke-width:34;pointer-events:stroke"/></g>';
    });
    return svg;
  }
  ...
  function butaneStep5Svg(expanded) {
    var selected = {};
    session.watchStep5CarbonIds.forEach(function (id) { selected[id] = true; });
    ...
    if (expanded) {
      ...
      butaneZigzagHydrogens.forEach(function (h) { svg += '<line x1="' + h[2] + '" y1="' + h[3] + '" x2="' + h[0] + '" y2="' + h[1] + '"/>'; });
      svg += '</g>';
      butaneZigzagHydrogens.forEach(function (h) {
        var tx = h[0] + (h[0] < 100 ? -14 : h[0] > 590 ? 6 : -10);
        var ty = h[1] + (h[1] < 150 ? -3 : h[1] > 230 ? 18 : 7);
        svg += '<text class="hydrogen" x="' + tx + '" y="' + ty + '">H</text>';
      });
    ...
    butaneCarbonSkeletonGeometry.carbons.forEach(function (c, index) {
      svg += step5MarkerSvg(c, index, !!selected[c[0]], expanded);
    });
    svg += '</svg>';
    return svg;
  }
  ...
  function propanolStep7Svg() {
    ...
    svg += '<g aria-hidden="true" style="stroke:currentColor;stroke-width:4;stroke-linecap:round">';
    svg += '<line x1="90" y1="220" x2="245" y2="120"/>';
    svg += '<line x1="245" y1="120" x2="400" y2="220"/>';
    svg += '<line x1="400" y1="220" x2="525" y2="155"/>';
    svg += '<line x1="555" y1="155" x2="615" y2="155"/>';
    svg += '</g>';
    svg += '<text data-step7-atom="O" x="526" y="168" style="font-size:40px;font-weight:900">O</text>';
    svg += '<text data-step7-atom="H" x="620" y="168" style="font-size:40px;font-weight:900">H</text>';
    svg += '<text x="78" y="275" style="font-size:20px;font-weight:700">carbon 1</text>';
    svg += '<text x="218" y="92" style="font-size:20px;font-weight:700">carbon 2</text>';
    svg += '<text x="372" y="274" style="font-size:20px;font-weight:700">carbon 3</text>';
    svg += '</svg>';
    return svg;
  }
  ...
  function bindCarbonTargets() {
    panel.querySelectorAll("[data-carbon-id]").forEach(function (node) {
      function choose() {
        if (watchSession && watchSession.paused) { announce("Watch is paused. Press Pause again to resume."); return; }
        ...
        } else if (result.reason === "already_tapped") announce("You already counted that carbon. Find a different carbon.");
      }
  ...
  function step2FeedbackForStoredChoice(step) {
    ...
    return session.watchStep2Prediction === step.prediction.answer ? step.prediction.correctFeedback : step.prediction.repairFeedback;
  }
  ...
  function showStep3SuccessInPlace(button, result) {
    ...
    if (svg && !svg.querySelector(".implied-h-ghost")) {
      [["70","197"],["118","105"],["118","287"]].forEach(function (point) {
  ...
  function reopenCompletedWatch() {
    ...
    watchSession = Watch.createWatchSession(Slice.WATCH_SEQUENCE, { timestamp: Date.now() });
    Watch.begin(watchSession, Slice.WATCH_SEQUENCE, Date.now());
    while (watchSession.currentIndex < Slice.WATCH_SEQUENCE.steps.length - 1) Watch.next(watchSession, Slice.WATCH_SEQUENCE, Date.now());
    Slice.syncWatchPhase(session, watchSession.currentIndex);
  }
  ...
  function renderWatchStep2() {
    ...
    var predictionGrid = choiceButtons(step.prediction.choices, function (choiceId, button) {
      if (watchSession.paused) { announce("Watch is paused. Press Pause again to resume before answering."); return; }
      var result = Slice.submitWatchStep2Prediction(session, choiceId);
      if (!result.accepted) { if (result.reason === "already_answered") announce("Your prediction is already recorded. Use Next when you are ready."); return; }
  ...
  function renderWatchStep3Repair() {
    ...
    panel.innerHTML = '<h1>Switch views: count the bond slots</h1>' +
      ...
      '<div class="step3-slot occupied" data-step3-slot="occupied">C—C</div><div class="step3-slot open" data-step3-slot="open">open</div><div class="step3-slot open" data-step3-slot="open">open</div><div class="step3-slot open" data-step3-slot="open">open</div><div class="step3-slot-carbon">C</div></div>' +
    ...
    var repairChoices = choiceButtons(step.repair.choices, function (value, button) {
      ...
      panel.querySelectorAll(".step3-repair-choice").forEach(function (choice) { choice.classList.toggle("selected", choice === button); });
      var success = document.createElement("div");
  ...
  function renderWatchStep3(options) {
    ...
    panel.innerHTML = '<h1>Hide the carbon-bound H labels</h1>' + teacher(step.narration) +
    ...
    var choices = choiceButtons(step.interaction.choices, function (value, button) {
      ...
      if (!result.accepted) { if (result.reason === "already_answered") announce("This Watch interaction is already complete. Use Next when you are ready."); return; }
      announce(result.feedback);
      if (!result.correct && result.repairRequired) { renderWatchStep3Repair(); return; }
  ...
  function renderWatchStep4(options) {
    ...
    var allCollapsed = session.watchStep4Complete;
    setPhase(repairMode ? "Watch · I Do · Step 4 · same-position repair" : "Watch · I Do · Step 4",
      allCollapsed ? "All four carbon labels are now abbreviated. The carbon atoms and bonds are still there." :
      repairMode ? "Same position, same bonds, same carbon. Watch the label change without moving the atom." :
      "Watch the first two carbon labels become bond-line positions, then make the prediction.");

    panel.innerHTML = '<h1>Hide the carbon letters one carbon at a time</h1>' + teacher(step.narration) +
      '<div class="prompt-card vocabulary-card"><div class="eyebrow">New term</div><h2>Line ends and vertices</h2><p>' + escapeHtml(step.vocabulary.vertex) + '</p></div>' +
      '<div class="watch-stage">' + butaneCarbonCollapseSvg(allCollapsed, repairMode, replayVisual) + '</div>' +
      (repairMode ? '<div id="step4RepairFocus" tabindex="-1" class="prediction-feedback"><strong>Look at the same corner:</strong> ' + escapeHtml(step.repair.narration) + '</div>' : '') +
      '<div class="prompt-card"><div class="eyebrow">Prediction pause</div><h2 id="step4PredictionPrompt">' + escapeHtml(step.prediction.prompt) + '</h2></div>';

    var predictionChoices = choiceButtons(step.prediction.choices, function (choiceId) {
      if (watchSession.paused) { announce("Watch is paused. Resume before answering."); return; }
      var result = Slice.submitWatchStep4Prediction(session, choiceId);
      if (!result.accepted) { if (result.reason === "already_answered") announce("This prediction is already complete. Use Next when you are ready."); return; }
      announce(result.feedback);
      speak(result.feedback);
      renderWatchStep4();
      if (result.correct) {
        var doneFocus = document.getElementById("step4CompleteFocus"); if (doneFocus) doneFocus.focus();
      } else {
        var repairFocus = document.getElementById("step4RepairFocus"); if (repairFocus) repairFocus.focus();
      }
    });
    predictionChoices.classList.add("step4-prediction-grid");
    predictionChoices.setAttribute("role", "group");
    predictionChoices.setAttribute("aria-labelledby", "step4PredictionPrompt");
    predictionChoices.querySelectorAll("button").forEach(function (button) {
      button.setAttribute("aria-pressed", "false");
      var label = button.textContent.trim();
      var selected = (session.watchStep4Prediction === "yes" && label === "Yes") ||
        (session.watchStep4Prediction === "no_corner_carbon" && label === "No, the corner now stands for the carbon") ||
        (session.watchStep4Prediction === "unsure" && label === "I am not sure yet");
    ...
    if (!allCollapsed) {
      if (repairMode) {
        gateChoiceGridUntilAnimationEnd(predictionChoices, panel.querySelector('[data-step4-toggle-carbon="C2"]'), "step4ToggleCarbon", "Same position, same bonds, same carbon. Now answer the prediction again.");
      } else {
        gateChoiceGridUntilAnimationEnd(predictionChoices, panel.querySelector('[data-step4-label="C2"]'), "step4HideCarbon", "The first two carbon labels are now abbreviated. Make your prediction.");
      }
    }

    if (allCollapsed) {
      var success = document.createElement("div");
      success.id = "step4CompleteFocus";
      success.tabIndex = -1;
      success.className = "success-box";
      success.setAttribute("role", "status");
      success.textContent = "Same positions, same bonds, same four carbons. The labels are abbreviated into two line ends and two vertices.";
      panel.appendChild(success);
    }
    var stage = panel.querySelector(".step4-stage");
    if (stage && watchSession.paused) stage.classList.add("is-paused");
    backBtn.disabled = watchSession.paused;
    replayBtn.disabled = watchSession.paused;
    pauseBtn.disabled = false;
    pauseBtn.textContent = watchSession.paused ? "Resume" : "Pause";
    ...
    if (allCollapsed && session.watchStep4Complete) {
      gateControlUntilAnimationEnd(nextBtn, panel.querySelector('[data-step4-label="C4"]'), "step4HideCarbon", "All four carbon labels are now abbreviated. You control when to move on.");
    }
  }

  function setStep5Feedback(text, success) {
    var feedback = document.getElementById("step5Feedback");
    if (!feedback) {
      feedback = document.createElement("div");
      feedback.id = "step5Feedback";
      feedback.setAttribute("role", "status");
      panel.appendChild(feedback);
    }
    feedback.className = success ? "success-box" : "prediction-feedback";
    feedback.textContent = text;
  }

  function updateStep5SelectionInPlace(node, result) {
    var id = node.getAttribute("data-step5-carbon");
    node.classList.add("selected");
    node.setAttribute("aria-pressed", "true");
    node.setAttribute("aria-label", node.getAttribute("aria-label").replace(/, selected$/, "") + ", selected");
    var marker = panel.querySelector('[data-step5-marker="' + id + '"]');
    if (marker) marker.classList.add("selected");
    var progress = document.getElementById("step5Progress");
    if (progress) progress.textContent = result.count + " of 4 carbons found";
    if (result.feedback) setStep5Feedback(result.feedback, result.stepComplete);
    status.textContent = result.stepComplete ? "All four carbon positions found. Compare the two notations or move on." : result.count + " of 4 carbon positions found.";
    nextBtn.disabled = !session.watchStep5Complete || watchSession.paused || watchFinished;
    if (result.stepComplete) {
      var compare = document.getElementById("step5CompareButton");
      if (!compare) {
        compare = document.createElement("button");
        compare.type = "button";
        compare.id = "step5CompareButton";
        compare.className = "choice-btn";
        compare.textContent = "Show expanded view";
        compare.addEventListener("click", toggleStep5Comparison);
  ...
  function bindStep5Targets() {
    panel.querySelectorAll("[data-step5-carbon]").forEach(function (node) {
      function choose() {
        if (watchSession.paused) { announce("Watch is paused. Resume before tapping carbon positions."); return; }
        var result = Slice.tapWatchStep5Carbon(session, node.getAttribute("data-step5-carbon"));
        if (result.accepted) {
          announce(result.feedback || result.count + " of 4 carbons found.");
          updateStep5SelectionInPlace(node, result);
        } else if (result.reason === "already_tapped") announce("You already found that carbon position.");
      }
      node.addEventListener("click", choose);
      node.addEventListener("keydown", function (event) {
        if (event.key === "Enter" || event.key === " ") { event.preventDefault(); choose(); }
    ...
    panel.querySelectorAll("[data-step5-bond]").forEach(function (node) {
      function chooseBond() {
        if (watchSession.paused) { announce("Watch is paused. Resume before answering."); return; }
        var result = Slice.tapWatchStep5Carbon(session, node.getAttribute("data-step5-bond"));
        if (result && result.feedback) { announce(result.feedback); updateStep5FeedbackInPlace(result.feedback); }
      }
      node.addEventListener("click", chooseBond);
      node.addEventListener("keydown", function (event) {
        if (event.key === "Enter" || event.key === " ") { event.preventDefault(); chooseBond(); }
      });
    });
  }

  function toggleStep5Comparison() {
    if (watchSession.paused) return;
    var result = Slice.toggleWatchStep5View(session);
    if (!result.accepted) return;
    renderWatchStep5();
    var compare = document.getElementById("step5CompareButton");
    if (compare) compare.focus();
  }

  function renderWatchStep5() {
    clearStep4GateListeners();
    var step = Slice.WATCH_SEQUENCE.steps[4];
    var expanded = session.watchStep5View === "expanded";
    setPhase(expanded ? "Watch · I Do · Step 5 · expanded comparison" : "Watch · I Do · Step 5",
      session.watchStep5Complete ? "Exactly four carbon positions found. Compare the same numbered carbons across views." : "Find every unlabeled carbon position: both line ends and both vertices.");
    panel.innerHTML = '<h1>Finished bond-line structure</h1>' + teacher(step.narration) +
      '<div class="watch-stage">' + butaneStep5Svg(expanded) + '</div>' +
      '<div class="prompt-card"><div class="eyebrow">Immediate interaction</div><h2 id="step5TapPrompt">' + escapeHtml(step.interaction.prompt) + '</h2><p id="step5Progress" class="progress-copy">' + session.watchStep5CarbonIds.length + ' of 4 carbons found</p></div>';
    if (session.watchStep5Complete) {
      setStep5Feedback(step.interaction.completeFeedback, true);
      var compare = document.createElement("button");
      compare.type = "button";
      compare.id = "step5CompareButton";
      compare.className = "choice-btn";
      compare.textContent = expanded ? "Show bond-line view" : "Show expanded view";
      compare.addEventListener("click", toggleStep5Comparison);
      panel.appendChild(compare);
    }
    bindStep5Targets();
  ...
  function renderWatchStep6() {
    ...
    panel.innerHTML = '<h1>Recover the hydrogens that are not written</h1>' + teacher(step.narration) +
      '<div class="watch-stage">' + butaneStep6Svg() + '</div>' +
      '<div class="prompt-card"><div class="eyebrow">Visible bond order + implied H = 4</div><h2 id="step6Prompt" tabindex="-1">' + escapeHtml(interaction.prompt) + '</h2></div>';
    var choices = choiceButtons(interaction.choices, function (value, button) {
      ...
      speak(result.feedback);
      if (previousTarget === "C3" && !session.watchStep6Complete) {
        renderWatchStep6();
        var prompt = document.getElementById("step6Prompt");
        if (prompt) prompt.focus();
        return;
      }
    ...
    choices.setAttribute("role", "group");
    choices.setAttribute("aria-labelledby", "step6Prompt");
    panel.appendChild(choices);
    if (session.watchStep6Complete) {
      setStep6Feedback({ correct: true, feedback: "Carbon 3 has two implied hydrogens. Carbon 4 has three implied hydrogens. Both counts come from reaching four total bonds." });
    }
    backBtn.disabled = watchSession.paused;
    replayBtn.disabled = watchSession.paused;
    pauseBtn.disabled = false;
    pauseBtn.textContent = watchSession.paused ? "Resume" : "Pause";
    nextBtn.disabled = !session.watchStep6Complete || watchSession.paused || watchFinished;
  }

  function setStep7Feedback(result) {
    var feedback = document.getElementById("step7Feedback");
    if (!feedback) {
      feedback = document.createElement("div");
      feedback.id = "step7Feedback";
      feedback.setAttribute("role", "status");
      panel.appendChild(feedback);
    }
    feedback.className = result.correct ? "success-box" : "prediction-feedback";
  ...
  function updateStep7PredictionInPlace(button, result) {
    panel.querySelectorAll(".step7-choice").forEach(function (choice) {
      choice.classList.toggle("selected", choice === button);
      choice.setAttribute("aria-pressed", choice === button ? "true" : "false");
    });
    setStep7Feedback(result);
    status.textContent = "Prediction recorded. Oxygen remains explicitly labeled in bond-line notation.";
    nextBtn.disabled = watchSession.paused || watchFinished;
  }

  function renderWatchStep7() {
    clearStep4GateListeners();
    var step = Slice.WATCH_SEQUENCE.steps[6];
    setPhase("Watch · I Do · Step 7", session.watchStep7Complete ? "Prediction recorded. You control when to move on." : "Notice which atoms remain written before making the prediction.");
    panel.innerHTML = '<h1>Heteroatoms stay explicit</h1>' + teacher(step.narration) +
      '<div class="prompt-card vocabulary-card"><div class="eyebrow">New term</div><h2>Heteroatom</h2><p>' + escapeHtml(step.definition) + '</p></div>' +
      '<div class="watch-stage">' + propanolStep7Svg() + '</div>' +
      '<div class="prompt-card"><div class="eyebrow">Prediction</div><h2 id="step7PredictionPrompt">' + escapeHtml(step.prediction.prompt) + '</h2></div>' +
      '<div class="prompt-card"><div class="eyebrow">Do not overgeneralize the shortcut</div><p>' + escapeHtml(step.misconceptionGuard) + '</p></div>';
    var choices = choiceButtons(step.prediction.choices, function (choiceId, button) {
      if (watchSession.paused) { announce("Watch is paused. Resume before answering."); return; }
      var result = Slice.submitWatchStep7Prediction(session, choiceId);
      if (!result.accepted) { if (result.reason === "already_answered") announce("Your Step 7 prediction is already recorded. Use Next when you are ready."); return; }
      announce(result.feedback);
      speak(result.feedback);
      updateStep7PredictionInPlace(button, result);
    });
    choices.classList.add("step7-choice-grid");
    choices.setAttribute("role", "group");
    choices.setAttribute("aria-labelledby", "step7PredictionPrompt");
    choices.querySelectorAll("button").forEach(function (button) {
      button.classList.add("step7-choice");
      button.setAttribute("aria-pressed", "false");
      var label = button.textContent.trim();
      var selected = (session.watchStep7Prediction === "yes" && label === "Yes") ||
        (session.watchStep7Prediction === "no" && label === "No") ||
        (session.watchStep7Prediction === "unsure" && label === "I am not sure yet");
      if (selected) { button.classList.add("selected"); button.setAttribute("aria-pressed", "true"); }
    });
    panel.appendChild(choices);
    if (session.watchStep7Complete) {
      var correctStored = session.watchStep7Prediction === step.prediction.answer;
      setStep7Feedback({ correct: correctStored, feedback: correctStored ? step.prediction.feedback : step.prediction.correctionFeedback });
    }
    backBtn.disabled = watchSession.paused;
    replayBtn.disabled = watchSession.paused;
    pauseBtn.disabled = false;
    pauseBtn.textContent = watchSession.paused ? "Resume" : "Pause";
    nextBtn.disabled = !session.watchStep7Complete || watchSession.paused || watchFinished;
  }

  function renderWatch() {
    ensureWatchSession();
    controls.hidden = false;
    if (watchSession.currentIndex === 6 || session.phase === "watch_step_7") renderWatchStep7();
    else if (watchSession.currentIndex === 5 || session.phase === "watch_step_6") renderWatchStep6();
    else if (watchSession.currentIndex === 4 || session.phase === "watch_step_5") renderWatchStep5();
    else if (watchSession.currentIndex === 3 || session.phase === "watch_step_4") renderWatchStep4();
    else if (watchSession.currentIndex === 2 || session.phase === "watch_step_3") renderWatchStep3();
  ...
  function renderSliceComplete() {
    clearStep4GateListeners();
    controls.hidden = false;
    setPhase("Watch · Step 7 complete", "Slice 7 stops here. No mastery claim has been made.");
    panel.innerHTML = '<h1>Step 7 is working.</h1>' +
      teacher("You saw that carbon uses special shorthand, while oxygen stays explicitly labeled and an attached heteroatom hydrogen may also remain written. Nothing here counts as independent mastery evidence.") +
      '<div class="success-box">Runtime Slice 7 complete: heteroatoms remain explicit instead of silently turning into carbon.</div>';
  ...
  function render() {
    ...
    else if (/^watch_step_[1-7]$/.test(session.phase)) renderWatch();
  }
  ...
  nextBtn.addEventListener("click", function () {
    ...
    if (watchSession.currentIndex === 0) {
      ...
      var r1 = Watch.next(watchSession, Slice.WATCH_SEQUENCE, Date.now());
      if (r1.changed && watchSession.currentIndex === 1) { Slice.syncWatchPhase(session, 1); announce("Watch Step 2. The carbon skeleton is now emphasized while the hydrogens remain present."); renderWatch(); }
      return;
    }
    if (watchSession.currentIndex === 1) {
      if (!session.watchStep2Complete) return;
      var r2 = Watch.next(watchSession, Slice.WATCH_SEQUENCE, Date.now());
      if (r2.changed && watchSession.currentIndex === 2) { Slice.syncWatchPhase(session, 2); announce("Watch Step 3. The carbon-bound hydrogen labels are no longer written, but the hydrogens are still implied."); renderWatch(); }
      return;
    }
    if (watchSession.currentIndex === 2) {
      if (!session.watchStep3Complete) return;
      var r3 = Watch.next(watchSession, Slice.WATCH_SEQUENCE, Date.now());
      if (r3.changed && watchSession.currentIndex === 3) { Slice.syncWatchPhase(session, 3); announce("Watch Step 4. Carbon labels are shortening into line ends and vertices without removing any carbon atoms."); renderWatch(); }
      return;
    }
    if (watchSession.currentIndex === 3) {
      ...
      if (r4.changed && watchSession.currentIndex === 4) { Slice.syncWatchPhase(session, 4); announce("Watch Step 5. The finished zig-zag has three bond lines but four carbon positions. Find the two ends and two vertices."); renderWatch(); }
      return;
    }
    if (watchSession.currentIndex === 4) {
      if (!session.watchStep5Complete || nextBtn.disabled) return;
      var r5 = Watch.next(watchSession, Slice.WATCH_SEQUENCE, Date.now());
      if (r5.changed && watchSession.currentIndex === 5) { Slice.syncWatchPhase(session, 5); announce("Watch Step 6. Recover the hydrogens that are implied by carbon's visible bond order."); renderWatch(); }
      return;
    }
    if (watchSession.currentIndex === 5) {
      if (!session.watchStep6Complete || nextBtn.disabled) return;
      var r6 = Watch.next(watchSession, Slice.WATCH_SEQUENCE, Date.now());
      if (r6.changed && watchSession.currentIndex === 6) { Slice.syncWatchPhase(session, 6); announce("Watch Step 7. Carbon shorthand stops at heteroatoms, so oxygen stays explicitly labeled."); renderWatch(); }
      return;
    }
    if (watchSession.currentIndex === 6) {
      if (!session.watchStep7Complete || nextBtn.disabled) return;
      var r7 = Watch.next(watchSession, Slice.WATCH_SEQUENCE, Date.now());
      if (r7.reason === "completed") { watchFinished = true; announce("Watch Step 7 complete."); render(); }
    }
  });
  ...
  backBtn.addEventListener("click", function () {
    ...
    if (result.changed) { Slice.syncWatchPhase(session, watchSession.currentIndex); announce("Back one Watch step."); renderWatch(); }
  });

  replayBtn.addEventListener("click", function () {
    if (!watchSession) return;
    if (watchSession.paused) { announce("Watch is paused. Resume before replaying this step."); return; }
    ...
    if (session.phase === "watch_step_5") renderWatchStep5();
    if (session.phase === "watch_step_6") renderWatchStep6();
    if (session.phase === "watch_step_7") renderWatchStep7();
  ...
  pauseBtn.addEventListener("click", function () {
    ...
    if (session.phase === "watch_step_3") {
      var step3Stage = panel.querySelector(".step3-stage"); if (step3Stage) step3Stage.classList.toggle("is-paused", watchSession.paused);
      backBtn.disabled = watchSession.paused; replayBtn.disabled = watchSession.paused; nextBtn.disabled = watchSession.paused || !session.watchStep3Complete || watchFinished; pauseBtn.textContent = watchSession.paused ? "Resume" : "Pause"; return;
    }
    ...
    if (session.phase === "watch_step_5") {
      backBtn.disabled = watchSession.paused; replayBtn.disabled = watchSession.paused; nextBtn.disabled = watchSession.paused || !session.watchStep5Complete || watchFinished;
      var compare = document.getElementById("step5CompareButton"); if (compare) compare.disabled = watchSession.paused;
      pauseBtn.textContent = watchSession.paused ? "Resume" : "Pause"; return;
    }
    if (session.phase === "watch_step_6") {
      backBtn.disabled = watchSession.paused; replayBtn.disabled = watchSession.paused; nextBtn.disabled = watchSession.paused || !session.watchStep6Complete || watchFinished;
      panel.querySelectorAll(".choice-grid button").forEach(function (button) { button.disabled = watchSession.paused; });
      pauseBtn.textContent = watchSession.paused ? "Resume" : "Pause"; return;
    }
    if (session.phase === "watch_step_7") {
      backBtn.disabled = watchSession.paused; replayBtn.disabled = watchSession.paused; nextBtn.disabled = watchSession.paused || !session.watchStep7Complete || watchFinished;
      panel.querySelectorAll(".step7-choice").forEach(function (button) { button.disabled = watchSession.paused; });
      pauseBtn.textContent = watchSession.paused ? "Resume" : "Pause"; return;
    }
    renderWatch();
  });
