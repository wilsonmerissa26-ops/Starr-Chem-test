(function () {
  ...
  function showStep3SuccessInPlace(button, result) {
    ...
    if (svg && !svg.querySelector(".implied-h-ghost")) {
      [["70","197"],["118","105"],["118","287"]].forEach(function (point) {
        ...
        ghost.setAttribute("x", point[0]); ghost.setAttribute("y", point[1]); ghost.textContent = "H";
  ...
  function renderWatchStep1() {
    ...
    bindCarbonTargets();
    backBtn.disabled = true; replayBtn.disabled = false; pauseBtn.disabled = false;
  ...
  function renderWatchStep2() {
    ...
    var predictionGrid = choiceButtons(step.prediction.choices, function (choiceId, button) {
      ...
      updateStep2PredictionInPlace(button, result); announce(result.feedback); speak(result.feedback);
    });
    predictionGrid.classList.add("prediction-grid"); predictionGrid.setAttribute("role", "group"); predictionGrid.setAttribute("aria-labelledby", "step2PredictionPrompt");
    predictionGrid.querySelectorAll("button").forEach(function (button) {
      button.classList.add("prediction-choice"); button.setAttribute("aria-pressed", "false");
      var label = button.textContent.trim(), stored = session.watchStep2Prediction;
    ...
    if (existing) {
      var feedback = document.createElement("div"); feedback.id = "step2PredictionFeedback"; feedback.className = "prediction-feedback"; feedback.setAttribute("role", "status"); feedback.textContent = existing; panel.appendChild(feedback);
    }
    backBtn.disabled = watchSession.paused; replayBtn.disabled = false; pauseBtn.disabled = false; pauseBtn.textContent = watchSession.paused ? "Resume" : "Pause";
    nextBtn.disabled = !session.watchStep2Complete || watchSession.paused || watchFinished;
  }
  ...
  function renderWatchStep3Repair() {
    ...
    var repairChoices = choiceButtons(step.repair.choices, function (value, button) {
      if (watchSession.paused) { announce("Watch is paused. Press Pause again to resume before answering."); return; }
      var result = Slice.submitWatchStep3Repair(session, value); if (!result.accepted) return; announce(result.feedback);
      if (!result.correct) {
        var feedback = panel.querySelector(".step3-repair-feedback");
        if (!feedback) { feedback = document.createElement("div"); feedback.className = "prediction-feedback step3-repair-feedback"; feedback.setAttribute("role", "status"); panel.appendChild(feedback); }
        feedback.textContent = result.feedback; return;
      }
      panel.querySelectorAll(".step3-repair-choice").forEach(function (choice) { choice.classList.toggle("selected", choice === button); });
      var success = document.createElement("div"); success.id = "step3Feedback"; success.className = "success-box"; success.setAttribute("role", "status"); success.textContent = result.feedback; panel.appendChild(success);
      status.textContent = "Three implied hydrogens reconnected. You control when to move on."; nextBtn.disabled = watchSession.paused || watchFinished; speak(result.feedback);
    });
    repairChoices.querySelectorAll("button").forEach(function (button) { button.classList.add("step3-repair-choice"); });
    repairChoices.setAttribute("role", "group"); repairChoices.setAttribute("aria-labelledby", "step3RepairPrompt"); panel.appendChild(repairChoices);
    ...
    var prompt = document.getElementById("step3RepairPrompt"); if (prompt) prompt.focus();
  }
  ...
  function renderWatchStep3(options) {
    ...
    var choices = choiceButtons(step.interaction.choices, function (value, button) {
      ...
      showStep3SuccessInPlace(button, result); speak(result.feedback);
    });
    choices.classList.add("step3-choice-grid"); choices.setAttribute("role", "group"); choices.setAttribute("aria-labelledby", "step3HydrogenPrompt");
    choices.querySelectorAll("button").forEach(function (button) {
      button.classList.add("step3-choice"); button.setAttribute("aria-pressed", String(session.watchStep3Complete && Number(button.textContent.trim()) === 3));
    ...
    if (session.watchStep3Complete) {
      var existing = document.createElement("div"); existing.id = "step3Feedback"; existing.className = "success-box"; existing.setAttribute("role", "status"); existing.textContent = "Yes. The three remaining bond slots reconnect to three implied hydrogens."; panel.appendChild(existing);
    }
    var step3Stage = panel.querySelector(".step3-stage"); if (step3Stage && watchSession.paused) step3Stage.classList.add("is-paused");
    backBtn.disabled = watchSession.paused; replayBtn.disabled = watchSession.paused; pauseBtn.disabled = false; pauseBtn.textContent = watchSession.paused ? "Resume" : "Pause";
    nextBtn.disabled = !session.watchStep3Complete || watchSession.paused || watchFinished;
  }
  ...
  function renderWatchStep4(options) {
    ...
    predictionChoices.classList.add("step4-prediction-grid"); predictionChoices.setAttribute("role", "group"); predictionChoices.setAttribute("aria-labelledby", "step4PredictionPrompt");
  ...
  function renderWatch() {
    ensureWatchSession(); controls.hidden = false;
    if (watchSession.currentIndex === 3 || session.phase === "watch_step_4") renderWatchStep4();
  ...
  function renderSliceComplete() {
    ...
    panel.innerHTML = '<h1>Step 4 is working.</h1>' +
      teacher("You watched the carbon labels become line ends and vertices without changing the carbon positions or bonds. If the disappearing C looked like a disappearing atom, we toggled the same label on and off over the same corner until the role stayed clear. Nothing here counts as independent mastery evidence.") +
      '<div class="success-box">Runtime Slice 4 complete: carbon labels are now abbreviated while molecular identity is preserved.</div>';
    nextBtn.disabled = true; backBtn.disabled = false; replayBtn.disabled = false; pauseBtn.disabled = true;
  }
  ...
  function render() {
    ...
    else if (session.phase === "watch_step_1" || session.phase === "watch_step_2" || session.phase === "watch_step_3" || session.phase === "watch_step_4") renderWatch();
  }
  ...
  backBtn.addEventListener("click", function () {
    if (watchFinished) {
      ...
      announce("Back one Watch step."); renderWatch(); return;
    }
