(function () {
  ...
  function addMediaChangeListener(query, handler) {
    ...
    if (typeof query.addEventListener === "function") {
      ...
      return function () { if (typeof query.removeEventListener === "function") query.removeEventListener("change", handler); };
    }
    if (typeof query.addListener === "function") {
      query.addListener(handler);
      return function () { if (typeof query.removeListener === "function") query.removeListener(handler); };
    }
    return function () {};
  }
  ...
  function trackStep4GateCleanup(cleanup) { step4GateCleanups.push(cleanup); }
  ...
  function hasNamedAnimation(node, animationName) {
    ...
    return !!(computed && computed.animationName && computed.animationName !== "none" && String(computed.animationName).split(",").some(function (name) { return name.trim() === animationName; }));
  }
  ...
  function gateChoiceGridUntilAnimationEnd(grid, node, animationName, readyMessage) {
    ...
    function armForCurrentMotion() {
      ...
      if (hasNamedAnimation(node, animationName)) setEnabled(false); else setEnabled(true);
    }
    ...
    function onMotionChange(event) {
      if (cleaned || !node.isConnected) { cleanup(); return; }
      if (event && event.matches) release(); else armForCurrentMotion();
    }
  ...
  function gateControlUntilAnimationEnd(control, node, animationName, readyMessage) {
    ...
    function onMotionChange(event) {
      if (cleaned || !node.isConnected) { cleanup(); return; }
      if (event && event.matches) markComplete(); else armForCurrentMotion();
    }
    node.addEventListener("animationend", onAnimationFinished);
    node.addEventListener("animationcancel", onAnimationFinished);
    removeMotionListener = addMediaChangeListener(motionQuery, onMotionChange);
  ...
  function updateRepairFeedbackInPlace(repairId, text) {
    ...
    if (!feedback) {
      ...
      if (choices) panel.insertBefore(feedback, choices); else panel.appendChild(feedback);
    }
  ...
  function butaneStep6Svg() {
    ...
    butaneCarbonSkeletonGeometry.carbons.forEach(function(c,index){var teaching=c[0]==="C2",active=c[0]===target;svg+='<g data-step6-carbon="'+c[0]+'" class="step6-carbon'+(teaching?' teaching':'')+(active?' active':'')+'" aria-label="Carbon '+(index+1)+(teaching?', worked example':'')+(active?', selected question':'')+'">';if(teaching||active)svg+='<circle cx="'+c[1]+'" cy="'+c[2]+'" r="34" style="fill:none;stroke:currentColor;stroke-width:'+(active?'6':'3')+'"/>';svg+='<text x="'+(c[1]-6)+'" y="'+(c[2]<170?c[2]-40:c[2]+52)+'" style="font-weight:900">'+(index+1)+'</text></g>';});
    return svg+'</svg>';
  }

  function propanolStep7Svg() {
    var svg='<svg data-step7-visual class="molecule-svg step7-stage" viewBox="0 0 680 340" role="img" aria-label="Bond-line structure of 1-propanol. The three-carbon chain uses line ends and a vertex while oxygen and its attached hydrogen remain explicitly labeled O and H.">';
    svg+='<g aria-hidden="true" style="stroke:currentColor;stroke-width:4;stroke-linecap:round"><line x1="90" y1="220" x2="245" y2="120"/><line x1="245" y1="120" x2="400" y2="220"/><line x1="400" y1="220" x2="525" y2="155"/><line x1="555" y1="155" x2="615" y2="155"/></g>';
    svg+='<text data-step7-atom="O" x="526" y="168" style="font-size:40px;font-weight:900">O</text><text data-step7-atom="H" x="620" y="168" style="font-size:40px;font-weight:900">H</text><text x="78" y="275" style="font-size:20px;font-weight:700">carbon 1</text><text x="218" y="92" style="font-size:20px;font-weight:700">carbon 2</text><text x="372" y="274" style="font-size:20px;font-weight:700">carbon 3</text>';
    return svg+'</svg>';
  }

  function buteneStep8Svg() {
    var line='stroke:#312838;stroke-width:6;stroke-linecap:round';
    var thin='stroke:#6f6476;stroke-width:4;stroke-linecap:round';
    var svg='<svg data-step8-visual class="molecule-svg step8-stage" viewBox="0 0 680 420" role="img" aria-label="The same four-carbon alkene shown first with carbon labels and then as a bond-line structure. The carbon-carbon double bond remains two parallel lines. Carbon 2 has one double bond and one single bond, visible bond order three.">';
    svg+='<text x="40" y="34" style="font-size:18px;font-weight:800;fill:#6d6476">Before shorthand: carbon labels shown</text>';
    svg+='<g aria-hidden="true" style="'+thin+'"><line x1="130" y1="105" x2="245" y2="105"/><line x1="130" y1="119" x2="245" y2="119"/><line x1="285" y1="112" x2="410" y2="112"/><line x1="450" y1="112" x2="565" y2="112"/></g>';
    [["C1",95],["C2",250],["C3",415],["C4",570]].forEach(function(c){svg+='<text x="'+c[1]+'" y="124" style="font-size:34px;font-weight:900;fill:#241b2b">C</text>';});
    svg+='<text x="40" y="205" style="font-size:18px;font-weight:800;fill:#6d6476">Bond-line view: labels collapse, bond order stays visible</text>';
    svg+='<g data-step8-double-bond aria-hidden="true"><line x1="90" y1="320" x2="245" y2="240" style="'+line+'"/><line x1="97" y1="334" x2="252" y2="254" style="'+line+'"/></g>';
    svg+='<line x1="250" y1="247" x2="410" y2="320" style="'+line+'" aria-hidden="true"/><line x1="410" y1="320" x2="570" y2="245" style="'+line+'" aria-hidden="true"/>';
    svg+='<circle cx="250" cy="247" r="35" style="fill:none;stroke:#6f3aa8;stroke-width:5"/><text x="238" y="213" style="font-size:20px;font-weight:900;fill:#4a236f">C2</text>';
    svg+='<text x="120" y="382" style="font-size:18px;font-weight:800;fill:#4a236f">double bond = 2</text><text x="320" y="382" style="font-size:18px;font-weight:800;fill:#4a236f">single bond = 1</text>';
    return svg+'</svg>';
  }
  ...
  function setStep6Feedback(result){var feedback=document.getElementById("step6Feedback");if(!feedback){feedback=document.createElement("div");feedback.id="step6Feedback";feedback.setAttribute("role","status");panel.appendChild(feedback);}feedback.className=result.correct?"success-box":"prediction-feedback";feedback.innerHTML=escapeHtml(result.feedback||"");if(result.formula)feedback.innerHTML+='<div class="support-note"><strong>'+escapeHtml(result.formula)+'</strong></div>';}
  function renderWatchStep6(){clearStep4GateListeners();var step=Slice.WATCH_SEQUENCE.steps[5],targetKey=session.watchStep6Target==="C4"?"carbon4":"carbon3",interaction=step.interactions[targetKey];setPhase("Watch · I Do · Step 6",session.watchStep6Complete?"Both hidden-hydrogen counts recovered. You control when to move on.":session.watchStep6Target==="C3"?"Use visible bond order to recover the hidden hydrogens on carbon 3.":"Now apply the same total-bond count to the terminal carbon 4.");panel.innerHTML='<h1>Recover the hydrogens that are not written</h1>'+teacher(step.narration)+'<div class="watch-stage">'+butaneStep6Svg()+'</div><div class="prompt-card"><div class="eyebrow">Visible bond order + implied H = 4</div><h2 id="step6Prompt" tabindex="-1">'+escapeHtml(interaction.prompt)+'</h2></div>';var choices=choiceButtons(interaction.choices,function(value,button){if(watchSession.paused){announce("Watch is paused. Resume before answering.");return;}var previousTarget=session.watchStep6Target,result=Slice.submitWatchStep6Hydrogens(session,value);if(!result.accepted)return;announce(result.feedback);if(!result.correct){setStep6Feedback(result);return;}speak(result.feedback);if(previousTarget==="C3"&&!session.watchStep6Complete){renderWatchStep6();var prompt=document.getElementById("step6Prompt");if(prompt)prompt.focus();return;}panel.querySelectorAll(".choice-grid button").forEach(function(choice){choice.classList.toggle("selected",choice===button);});setStep6Feedback(result);status.textContent="Carbon 3 is CH2 and carbon 4 is CH3. You control when to move on.";nextBtn.disabled=watchSession.paused||watchFinished;});choices.setAttribute("role","group");choices.setAttribute("aria-labelledby","step6Prompt");panel.appendChild(choices);if(session.watchStep6Complete)setStep6Feedback({correct:true,feedback:"Carbon 3 has two implied hydrogens. Carbon 4 has three implied hydrogens. Both counts come from reaching four total bonds."});backBtn.disabled=watchSession.paused;replayBtn.disabled=watchSession.paused;pauseBtn.disabled=false;pauseBtn.textContent=watchSession.paused?"Resume":"Pause";nextBtn.disabled=!session.watchStep6Complete||watchSession.paused||watchFinished;}

  function setStep7Feedback(result){var feedback=document.getElementById("step7Feedback");if(!feedback){feedback=document.createElement("div");feedback.id="step7Feedback";feedback.setAttribute("role","status");panel.appendChild(feedback);}feedback.className=result.correct?"success-box":"prediction-feedback";feedback.textContent=result.feedback||"";}
  function updateStep7PredictionInPlace(button,result){panel.querySelectorAll(".step7-choice").forEach(function(choice){choice.classList.toggle("selected",choice===button);choice.setAttribute("aria-pressed",choice===button?"true":"false");});setStep7Feedback(result);status.textContent="Prediction recorded. Oxygen remains explicitly labeled in bond-line notation.";nextBtn.disabled=watchSession.paused||watchFinished;}
  function renderWatchStep7(){clearStep4GateListeners();var step=Slice.WATCH_SEQUENCE.steps[6];setPhase("Watch · I Do · Step 7",session.watchStep7Complete?"Prediction recorded. You control when to move on.":"Notice which atoms remain written before making the prediction.");panel.innerHTML='<h1>Heteroatoms stay explicit</h1>'+teacher(step.narration)+'<div class="prompt-card vocabulary-card"><div class="eyebrow">New term</div><h2>Heteroatom</h2><p>'+escapeHtml(step.definition)+'</p></div><div class="watch-stage">'+propanolStep7Svg()+'</div><div class="prompt-card"><div class="eyebrow">Prediction</div><h2 id="step7PredictionPrompt">'+escapeHtml(step.prediction.prompt)+'</h2></div><div class="prompt-card"><div class="eyebrow">Do not overgeneralize the shortcut</div><p>'+escapeHtml(step.misconceptionGuard)+'</p></div>';var choices=choiceButtons(step.prediction.choices,function(choiceId,button){if(watchSession.paused){announce("Watch is paused. Resume before answering.");return;}var result=Slice.submitWatchStep7Prediction(session,choiceId);if(!result.accepted){if(result.reason==="already_answered")announce("Your Step 7 prediction is already recorded. Use Next when you are ready.");return;}announce(result.feedback);speak(result.feedback);updateStep7PredictionInPlace(button,result);});choices.classList.add("step7-choice-grid");choices.setAttribute("role","group");choices.setAttribute("aria-labelledby","step7PredictionPrompt");choices.querySelectorAll("button").forEach(function(button){button.classList.add("step7-choice");button.setAttribute("aria-pressed","false");var label=button.textContent.trim(),selected=(session.watchStep7Prediction==="yes"&&label==="Yes")||(session.watchStep7Prediction==="no"&&label==="No")||(session.watchStep7Prediction==="unsure"&&label==="I am not sure yet");if(selected){button.classList.add("selected");button.setAttribute("aria-pressed","true");}});panel.appendChild(choices);if(session.watchStep7Complete){var correctStored=session.watchStep7Prediction===step.prediction.answer;setStep7Feedback({correct:correctStored,feedback:correctStored?step.prediction.feedback:step.prediction.correctionFeedback});}backBtn.disabled=watchSession.paused;replayBtn.disabled=watchSession.paused;pauseBtn.disabled=false;pauseBtn.textContent=watchSession.paused?"Resume":"Pause";nextBtn.disabled=!session.watchStep7Complete||watchSession.paused||watchFinished;}

  function setStep8Feedback(result){var feedback=document.getElementById("step8Feedback");if(!feedback){feedback=document.createElement("div");feedback.id="step8Feedback";feedback.setAttribute("role","status");panel.appendChild(feedback);}feedback.className=result.correct?"success-box":"prediction-feedback";feedback.textContent=result.feedback||"";}
  function renderWatchStep8(){clearStep4GateListeners();var step=Slice.WATCH_SEQUENCE.steps[7];setPhase("Watch · I Do · Step 8",session.watchStep8Complete?"Bond order recovered correctly. You control when to move on.":"Count bond order, not just neighboring atoms.");panel.innerHTML='<h1>Multiple bonds stay visible</h1>'+teacher(step.narration)+'<div class="watch-stage">'+buteneStep8Svg()+'</div><div class="prompt-card"><div class="eyebrow">Bond order matters</div><h2 id="step8Prompt">'+escapeHtml(step.interaction.prompt)+'</h2></div>';var choices=choiceButtons(step.interaction.choices,function(value,button){if(watchSession.paused){announce("Watch is paused. Resume before answering.");return;}var result=Slice.submitWatchStep8Hydrogen(session,value);if(!result.accepted)return;announce(result.feedback);setStep8Feedback(result);if(!result.correct)return;panel.querySelectorAll(".step8-choice").forEach(function(choice){choice.classList.toggle("selected",choice===button);choice.setAttribute("aria-pressed",choice===button?"true":"false");});status.textContent="Bond order 3 leaves one implied hydrogen. You control when to move on.";nextBtn.disabled=watchSession.paused||watchFinished;speak(result.feedback);});choices.classList.add("step8-choice-grid");choices.setAttribute("role","group");choices.setAttribute("aria-labelledby","step8Prompt");choices.querySelectorAll("button").forEach(function(button){button.classList.add("step8-choice");button.setAttribute("aria-pressed",String(session.watchStep8Complete&&Number(button.textContent.trim())===1));if(session.watchStep8Complete&&Number(button.textContent.trim())===1)button.classList.add("selected");});panel.appendChild(choices);if(session.watchStep8Complete)setStep8Feedback({correct:true,feedback:"Right. Bond order 3 leaves room for one C—H bond so carbon reaches four total bonds."});backBtn.disabled=watchSession.paused;replayBtn.disabled=watchSession.paused;pauseBtn.disabled=false;pauseBtn.textContent=watchSession.paused?"Resume":"Pause";nextBtn.disabled=!session.watchStep8Complete||watchSession.paused||watchFinished;}

  function renderWatch(){ensureWatchSession();controls.hidden=false;if(watchSession.currentIndex===7||session.phase==="watch_step_8")renderWatchStep8();else if(watchSession.currentIndex===6||session.phase==="watch_step_7")renderWatchStep7();else if(watchSession.currentIndex===5||session.phase==="watch_step_6")renderWatchStep6();else if(watchSession.currentIndex===4||session.phase==="watch_step_5")renderWatchStep5();else if(watchSession.currentIndex===3||session.phase==="watch_step_4")renderWatchStep4();else if(watchSession.currentIndex===2||session.phase==="watch_step_3")renderWatchStep3();else if(watchSession.currentIndex===1||session.phase==="watch_step_2")renderWatchStep2();else renderWatchStep1();}

  function renderSliceComplete(){clearStep4GateListeners();controls.hidden=false;setPhase("Watch · Step 8 complete","All eight I Do Watch steps are implemented. No mastery claim has been made.");panel.innerHTML='<h1>The full Watch sequence is working.</h1>'+teacher("You followed bond-line notation from fully written atoms through hidden carbon hydrogens, hidden carbon labels, heteroatoms, and multiple-bond order. This is still supported teaching, not independent evidence.")+'<div class="success-box">Watch complete. Next comes the supported concept check before Build Together.</div>';nextBtn.disabled=true;backBtn.disabled=false;replayBtn.disabled=false;pauseBtn.disabled=true;}

  function render(){if(watchFinished){renderSliceComplete();return;}if(session.phase==="orientation")renderOrientation();else if(session.phase==="gate_p1")renderGateP1();else if(session.phase==="repair_p1")renderRepairP1();else if(session.phase==="gate_p2")renderGateP2();else if(session.phase==="repair_p2")renderRepairP2();else if(/^watch_step_[1-8]$/.test(session.phase))renderWatch();}

  nextBtn.addEventListener("click",function(){if(!watchSession||watchSession.paused)return;
    ...
    if(watchSession.currentIndex===5){if(!session.watchStep6Complete||nextBtn.disabled)return;var r6=Watch.next(watchSession,Slice.WATCH_SEQUENCE,Date.now());if(r6.changed&&watchSession.currentIndex===6){Slice.syncWatchPhase(session,6);announce("Watch Step 7. Carbon shorthand stops at heteroatoms, so oxygen stays explicitly labeled.");renderWatch();}return;}
    if(watchSession.currentIndex===6){if(!session.watchStep7Complete||nextBtn.disabled)return;var r7=Watch.next(watchSession,Slice.WATCH_SEQUENCE,Date.now());if(r7.changed&&watchSession.currentIndex===7){Slice.syncWatchPhase(session,7);announce("Watch Step 8. Multiple bonds remain visible, so count bond order before inferring hydrogens.");renderWatch();}return;}
    if(watchSession.currentIndex===7){if(!session.watchStep8Complete||nextBtn.disabled)return;var r8=Watch.next(watchSession,Slice.WATCH_SEQUENCE,Date.now());if(r8.reason==="completed"){watchFinished=true;announce("Watch Step 8 complete.");render();}}
  });
  ...
  replayBtn.addEventListener("click",function(){if(!watchSession)return;if(watchSession.paused){announce("Watch is paused. Resume before replaying this step.");return;}if(watchFinished){reopenCompletedWatch();renderWatch();}Watch.replay(watchSession,Slice.WATCH_SEQUENCE,Date.now());if(session.phase==="watch_step_3"&&!session.watchStep3RepairActive)renderWatchStep3({suppressCompletionGhosts:true});if(session.phase==="watch_step_4")renderWatchStep4({replayVisual:!session.watchStep4RepairActive});if(session.phase==="watch_step_5")renderWatchStep5();if(session.phase==="watch_step_6")renderWatchStep6();if(session.phase==="watch_step_7")renderWatchStep7();if(session.phase==="watch_step_8")renderWatchStep8();announce("Replaying the current Watch step only.");speak(Slice.WATCH_SEQUENCE.steps[watchSession.currentIndex].narration);});

  pauseBtn.addEventListener("click",function(){if(!watchSession||watchFinished)return;var result=Watch.pause(watchSession,Slice.WATCH_SEQUENCE,Date.now());announce(result.reason==="paused"?"Watch paused. The visual will stay exactly here.":"Watch resumed.");
    if(session.phase==="watch_step_3"){var s3=panel.querySelector(".step3-stage");if(s3)s3.classList.toggle("is-paused",watchSession.paused);backBtn.disabled=watchSession.paused;replayBtn.disabled=watchSession.paused;nextBtn.disabled=watchSession.paused||!session.watchStep3Complete||watchFinished;pauseBtn.textContent=watchSession.paused?"Resume":"Pause";return;}
    if(session.phase==="watch_step_4"){var s4=panel.querySelector(".step4-stage");if(s4)s4.classList.toggle("is-paused",watchSession.paused);backBtn.disabled=watchSession.paused;replayBtn.disabled=watchSession.paused;if(watchSession.paused)nextBtn.disabled=true;else if(!session.watchStep4Complete||watchFinished)nextBtn.disabled=true;else{var finalCollapse=panel.querySelector('[data-step4-label="C4"]');nextBtn.disabled=!finalCollapse||finalCollapse.getAttribute("data-animation-gate-complete")!=="true";}pauseBtn.textContent=watchSession.paused?"Resume":"Pause";return;}
    if(session.phase==="watch_step_5"){backBtn.disabled=watchSession.paused;replayBtn.disabled=watchSession.paused;nextBtn.disabled=watchSession.paused||!session.watchStep5Complete||watchFinished;var compare=document.getElementById("step5CompareButton");if(compare)compare.disabled=watchSession.paused;pauseBtn.textContent=watchSession.paused?"Resume":"Pause";return;}
    if(session.phase==="watch_step_6"){backBtn.disabled=watchSession.paused;replayBtn.disabled=watchSession.paused;nextBtn.disabled=watchSession.paused||!session.watchStep6Complete||watchFinished;panel.querySelectorAll(".choice-grid button").forEach(function(button){button.disabled=watchSession.paused;});pauseBtn.textContent=watchSession.paused?"Resume":"Pause";return;}
    if(session.phase==="watch_step_7"){backBtn.disabled=watchSession.paused;replayBtn.disabled=watchSession.paused;nextBtn.disabled=watchSession.paused||!session.watchStep7Complete||watchFinished;panel.querySelectorAll(".step7-choice").forEach(function(button){button.disabled=watchSession.paused;});pauseBtn.textContent=watchSession.paused?"Resume":"Pause";return;}
    if(session.phase==="watch_step_8"){backBtn.disabled=watchSession.paused;replayBtn.disabled=watchSession.paused;nextBtn.disabled=watchSession.paused||!session.watchStep8Complete||watchFinished;panel.querySelectorAll(".step8-choice").forEach(function(button){button.disabled=watchSession.paused;});pauseBtn.textContent=watchSession.paused?"Resume":"Pause";return;}
    renderWatch();
  });
