(function () {
  ...
  if (!Slice || !Watch) throw new Error("Bond-Line Slice 1 dependencies are missing");
  ...
  function choiceButtons(choices, handler) {
    ...
    choices.forEach(function (choice) {
      ...
      button.addEventListener("click", function () { handler(value); });
  ...
  function renderSliceComplete() {
    controls.hidden = false;
    setPhase("Watch · Step 1 complete", "Slice 1 stops here. No mastery claim has been made.");
    panel.innerHTML =
      '<h1>Step 1 is working.</h1>' +
      teacher("You identified all four carbon atoms while every atom was still visible. The next Watch step will separate the carbon skeleton from the hydrogens. Nothing here counts as independent mastery evidence.") +
      '<div class="success-box">Runtime Slice 1 complete: orientation, both prerequisite gates, narrow repairs, and the first fully expanded butane interaction are wired.</div>';
  ...
  function render() {
    ...
    else if (session.phase === "watch_step_1") renderWatch();
  }

  nextBtn.addEventListener("click", function () {
    if (!watchSession || !session.watchStep1Complete || watchSession.paused) return;
    var result = Watch.next(watchSession, Slice.WATCH_SEQUENCE, Date.now());
    if (result.reason === "completed") {
      watchFinished = true;
      announce("Watch Step 1 complete.");
      render();
    }
  });
