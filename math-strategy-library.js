...
function percentCandidate(id,p,w,answer,features,steps,copy){
  return {strategyId:id,answer:answer,valid:close(answer,pctAnswer(p,w)),features:features,steps:steps||[],concept:(copy&&copy.concept)||'Percent means parts per hundred.',mentalRoute:(copy&&copy.mentalRoute)||'',hint:(copy&&copy.hint)||''};
}
...
function percentCandidates(problem){
  ...
  if(close(p,75)) {
    ...
    out.push(percentCandidate('percent_half_plus_quarter',p,w,half+quarter,{operationCount:3,benchmarkBonus:2,divisionDifficulty:cost.divisionDifficulty(w,2)+cost.divisionDifficulty(w,4),decimalComplexity:decimalComplexity([half,quarter]),mentalLoad:1},[
      step('half','What is 50% of '+w+'?',half,['halving'],'Take half.'),
      step('quarter','What is 25% of '+w+'?',quarter,['quartering'],'Take one fourth.'),
      step('combine','Combine 50% and 25%.',half+quarter,['add_friendly_chunks'],'Add the two friendly parts.')
    ],{mentalRoute:'75% = 50% + 25%.'}));
  }
  if(close(p,15)) {
    var five=ten/2;
    out.push(percentCandidate('percent_10_plus_5',p,w,ten+five,{operationCount:3,benchmarkBonus:2,divisionDifficulty:cost.divisionDifficulty(w,10),decimalComplexity:decimalComplexity([ten,five]),mentalLoad:1},[
      step('ten','What is 10% of '+w+'?',ten,['divide_by_10'],'10% is one tenth.'),
      step('five','What is half of '+ten+'?',five,['halving'],'5% is half of 10%.'),
      step('combine','Combine 10% and 5%.',ten+five,['add_friendly_chunks'],'Add the two parts.')
    ],{mentalRoute:'15% = 10% + 5%.'}));
  }
  ...
  if(near10>=10 && near10<=90 && !close(near10,p)) {
    ...
    if(absDiff<=4.0000001) {
      var anchor=ten*(near10/10);
      var corr=one*absDiff;
      var result=diff>0?anchor+corr:anchor-corr;
      var id='percent_'+near10+'_'+(diff>0?'plus':'minus')+'_'+absDiff;
      var multAnchor=near10/10;
      var multCorr=absDiff;
      out.push(percentCandidate(id,p,w,result,{
        operationCount:5,
        anchorAcquisition:0.1,
        divisionDifficulty:cost.divisionDifficulty(w,10)+cost.divisionDifficulty(w,100),
        multiplicationDifficulty:cost.multiplicationDifficulty(ten,multAnchor)+cost.multiplicationDifficulty(one,multCorr),
        decimalComplexity:decimalComplexity([ten,one,anchor,corr]),
        mentalLoad:2,
        compensationComplexity:diff<0?1:0.7
      },[
        step('ten','Find 10% of '+w+'.',ten,['divide_by_10'],'Use the 10% anchor.'),
        step('anchor','Build '+near10+'% from 10%.',anchor,['multiply_by_small_whole'],'Scale the 10% chunk.'),
        step('one','Find 1% of '+w+'.',one,['divide_by_100'],'Use the 1% correction chunk.'),
        step('correction','Build '+absDiff+'% from 1%.',corr,['multiply_by_small_whole'],'Make the small correction.'),
        step('adjust',(diff>0?'Add':'Subtract')+' the correction.',result,[diff>0?'add_friendly_chunks':'subtract_friendly_chunks'],'Adjust from the nearby ten.')
      ],{mentalRoute:p+'% = '+near10+'% '+(diff>0?'+ ':'- ')+absDiff+'%.'}));
    }
  }

  if(Number.isInteger(p) && p>0) {
    var oneScaled=one*p;
    out.push(percentCandidate('percent_1_then_scale',p,w,oneScaled,{
      operationCount:2,
      divisionDifficulty:cost.divisionDifficulty(w,100),
      multiplicationDifficulty:cost.multiplicationDifficulty(one,p),
      decimalComplexity:decimalComplexity([one,oneScaled]),
      mentalLoad:1,
      routeOverhead:(Number.isInteger(one)?1.15:3.0)+Math.max(0,4-Math.abs(p-Math.round(p/10)*10))*0.55
    },[
      step('one','Find 1% of '+w+'.',one,['divide_by_100'],'1% is one hundredth.'),
      step('scale','Scale that 1% chunk to '+p+'%.',oneScaled,['multiply_by_small_whole'],'Use the clean 1% chunk.')
    ],{mentalRoute:'Find 1%, then scale it to '+p+'%.'}));
  }
  ...
  if(p<100 && complement>0 && complement<=20 && Number.isInteger(complement)) {
    ...
    if(tensPart>0 && onesPart>0) {
      var tensValue=ten*(tensPart/10), onesValue=one*onesPart;
      var freeResult=w-tensValue-onesValue;
      out.push(percentCandidate('percent_100_minus_'+tensPart+'_minus_'+onesPart,p,w,freeResult,{
        operationCount:5,
        anchorAcquisition:0,
        divisionDifficulty:cost.divisionDifficulty(w,10)+cost.divisionDifficulty(w,100),
        multiplicationDifficulty:cost.multiplicationDifficulty(ten,tensPart/10)+cost.multiplicationDifficulty(one,onesPart),
        decimalComplexity:decimalComplexity([ten,one,tensValue,onesValue]),
        mentalLoad:2,
        compensationComplexity:1.3,
        routeOverhead:0.6
      },[
        step('whole','Start from 100%: '+w+'.',w,[],'100% is the whole.'),
        step('tens','Find '+tensPart+'% to remove.',tensValue,['divide_by_10','multiply_by_small_whole'],'Build the tens correction.'),
        step('ones','Find '+onesPart+'% to remove.',onesValue,['divide_by_100','multiply_by_small_whole'],'Build the small correction.'),
        step('subtract_tens','Subtract '+tensPart+'%.',w-tensValue,['subtract_friendly_chunks'],'Subtract from the free 100% anchor.'),
        step('subtract_ones','Subtract the remaining '+onesPart+'%.',freeResult,['subtract_friendly_chunks'],'Finish the correction.')
      ],{mentalRoute:p+'% = 100% - '+tensPart+'% - '+onesPart+'%.'}));
    }
  }

  out.push(percentCandidate('percent_formal_decimal',p,w,(p/100)*w,{
    operationCount:2,
    divisionDifficulty:cost.divisionDifficulty(p,100),
    multiplicationDifficulty:cost.multiplicationDifficulty(p/100,w),
    decimalComplexity:decimalComplexity([p/100,ans]),
    mentalLoad:1,
    routeOverhead:1.5
  },[
    step('decimal','Convert '+p+'% to '+(p/100)+'.',p/100,['place_value_decimal_shift'],'Percent means divide by 100.'),
    step('multiply','Multiply by '+w+'.',ans,['multiply_by_small_whole'],'Now multiply the decimal by the whole.')
  ],{mentalRoute:'Convert percent to a decimal and multiply.'}));
...
function generate(problem){
  if(problem.family==='percent_of_whole') return percentCandidates(problem);
  if(problem.family==='fraction_of_whole') return fractionCandidates(problem);
  if(problem.family==='what_percent_of') return whatPercentCandidates(problem);
