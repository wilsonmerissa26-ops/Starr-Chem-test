...
function plan(problem,options){
  ...
  var chosen=candidates[0],near=candidates.slice(1).filter(function(c){return c.cost-chosen.cost<=NEAR_TIE;});
  return{problem:p,answer:expected,candidates:candidates,chosenStrategyId:chosen.strategyId,chosenPlan:makePlan(chosen),nearTies:near};
}
return{plan:plan,expectedAnswer:expectedAnswer,NEAR_TIE:NEAR_TIE};
});
