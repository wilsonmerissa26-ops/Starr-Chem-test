(function(root,factory){
  ...
  if(typeof module==='object'&&module.exports){
    api=factory(require('../day1-adaptive-runtime.js'),require('../day1-problem-source-adapters.js'),require('../math-answer-checker.js'),require('../student-model-idk-router.js'));
    module.exports=api;
  }else{
    api=factory(root.Day1AdaptiveRuntime,root.Day1ProblemSourceAdapters,root.MathAnswerChecker,root.StudentModelIdkRouter);
...
})(typeof globalThis!=='undefined'?globalThis:this,function(Runtime,Adapters,AnswerChecker,StudentModel){
'use strict';
...
function save(){if(typeof localStorage!=='undefined')saveStateToStore(localStorage,state);}
...
function strategyLabel(){var cur=state&&state.current;return cur&&cur.plan?cur.plan.chosenStrategyId.replace(/^percent_/,'').replace(/_/g,' '):'';}
...
function enhancePractice(){
  if(!root.Day1AdaptiveReady||!document.getElementById('check')||!document.querySelector('.question'))return false;
...
function renderRepair(result,message){
  ...
  var lesson=result.lesson,check=result.checkItem||result.nextCheckItem;
...
function submitRepair(){
  ...
  if(r.action==='return_to_parent_prerequisite'||r.action==='teach_deeper_prerequisite'||r.action==='switch_representation'||r.action==='retry_prerequisite'){
    renderRepair({lesson:r.lesson||Runtime._lesson,checkItem:r.nextCheckItem,representationContent:r.representationContent},r.correct?{good:true,text:'Good. Move back up one level.'}:{good:false,text:'Not yet. We are staying with the smaller skill.'});
    if(!r.lesson&&state.current&&state.current.activePrerequisiteSkillId&&root.MathPrerequisiteContent){repairView.lesson=root.MathPrerequisiteContent.getLesson(state.current.activePrerequisiteSkillId);renderRepair(repairView);}
    return;
  }
...
function start(){
  if(bound||typeof document==='undefined'||!Runtime||!Adapters||!AnswerChecker||!StudentModel)return false;
...
return{
  ...
  start:start,stop:stop,inferProblem:inferProblem,matchExpected:matchExpected,
