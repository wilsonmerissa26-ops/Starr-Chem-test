...
eq(Hub.dayStatus(1, storage({
  "dr-merissa-day1-state-v1": saved({ chemistry: { masteryMet: true }, math: { statuses: {} }, nextSessionQueue: [] })
}), curricula), Hub.STATUS.OPEN, "Day 1 mastery becomes OPEN");
...
eq(Hub.dayStatus(1, storage({
  "dr-merissa-day1-state-v1": saved({ chemistry: { masteryMet: false }, math: { statuses: { algebra: "Developing" } }, nextSessionQueue: ["algebra"] })
}), curricula), Hub.STATUS.NEEDS_REVIEW, "Day 1 developing evidence becomes NEEDS REVIEW");
eq(Hub.dayStatus(2, storage({
