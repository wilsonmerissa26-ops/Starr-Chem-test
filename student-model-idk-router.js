...
// The original three stable values remain unchanged for backward compatibility.
// Unit 1 adds the remaining three meanings required by the frozen teaching contract.
...
function handleIdk(skill, reason, currentItemId, requiredSkillId, timestamp) {
  ...
  // Preserve the legacy ceiling behavior for the original three reasons so the
  // existing contract stays byte-for-byte meaningful to old callers/tests.
  // The three newly shipped reasons keep their semantic action even at the
  // scaffold ceiling: prerequisite gap, stuck-step repair, or modality switch.
...
// STEP 2: the smaller prerequisite/confirmation check.
