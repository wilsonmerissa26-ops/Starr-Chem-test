...
// Count carries in the aligned addition of the tenths and hundredths partial
// products used by a two-digit percent multiplication. Example:
// 0.69*32 = 19.20 + 2.88, which requires two carries when those partials
// are added. This is an arithmetic feature, not a case-specific label.
function alignedAdditionCarryCount(a,b,places){
  places=places==null?2:places;
  var scale=Math.pow(10,places);
  var x=Math.round(Math.abs(Number(a))*scale);
  var y=Math.round(Math.abs(Number(b))*scale);
  if(!Number.isFinite(x)||!Number.isFinite(y))return 0;
  var carry=0,count=0;
  while(x>0||y>0||carry>0){
    var sum=(x%10)+(y%10)+carry;
    carry=sum>=10?1:0;
    if(carry)count++;
...
function percentFormalCarryCount(percent,whole){
  percent=Number(percent);whole=Number(whole);
  if(!Number.isFinite(percent)||!Number.isFinite(whole))return 0;
  if(!Number.isInteger(percent)||percent<10||percent>=100||!Number.isInteger(whole))return 0;
  var tens=Math.floor(percent/10);
  var ones=percent%10;
  var tenthsPartial=(tens/10)*whole;
  var hundredthsPartial=(ones/100)*whole;
  return alignedAdditionCarryCount(tenthsPartial,hundredthsPartial,2);
}
...
return {
  ...
  addSubtractDifficulty: addSubtractDifficulty,alignedAdditionCarryCount:alignedAdditionCarryCount,
  percentFormalCarryCount:percentFormalCarryCount,candidateSkillIds: candidateSkillIds,
