(function (root, factory) {
  ...
  const lines = count => `<div class="print-lines">${Array.from({ length: count || 4 }, () => "<div></div>").join("")}</div>`;
  ...
  function practiceItem(item) {
    const answerLines = item.stage === "transfer" ? 5 : isFallback(item) ? 3 : 4;
    return `<article class="print-question" data-print-item="${esc(item.id)}"><div class="print-item-head"><strong>${esc(item.id)}</strong><span>${esc(stageLabel(item))}</span></div><p class="print-prompt">${esc(item.prompt)}</p>${lines(answerLines)}</article>`;
  }
...
@media print{
  ...
  .print-packet{font:10.5pt/1.28 Georgia,"Times New Roman",serif;color:#111;background:#fff}
  .print-cover{border-bottom:2px solid #111;padding-bottom:.12in;margin-bottom:.12in}
  .print-kicker{font:700 8pt/1.15 Arial,sans-serif;letter-spacing:.11em;text-transform:uppercase}
  .print-cover h1{font:700 18pt/1.12 Arial,sans-serif;margin:.05in 0}
  .print-cover h2{font:700 13pt/1.15 Arial,sans-serif;margin:.05in 0}
  ...
  .print-vocab-item{border:1px solid #bbb;padding:.055in;break-inside:avoid}
  .print-vocab-item p{margin:.025in 0 0}
  .print-page-start{break-before:page}
  .print-question,.print-key-item{break-inside:avoid;border:1px solid #bbb;padding:.075in;margin:0 0 .08in}
  .print-item-head{display:flex;justify-content:space-between;gap:.12in;font:8.5pt Arial,sans-serif;text-transform:uppercase;letter-spacing:.025em}
  .print-prompt{font-weight:600;margin:.045in 0 .055in}
  .print-worked-answer,.print-answer{border-left:3px solid #444;padding-left:.07in;margin-top:.055in}
  .print-worked-answer p,.print-answer p{margin:.025in 0 0}
  .print-lines{margin-top:.035in}
  .print-lines div{height:.23in;border-bottom:1px solid #bbb}
}
`;
