...
let result = vocabulary.submit("acid acid acid", "This application is nonsense but contains acid and is definitely long enough.");
assert.equal(result.correct, false);
assert.equal(result.preservedDefinition, "acid acid acid");
result = vocabulary.submit("A species that donates a proton", "NH4+ is the acid because it donates H+ to water.");
assert(result.correct && result.retrievalPending && !result.masteryAwarded);
vocabulary.beginRetrieval();
result = vocabulary.submit("It donates H+ to a base", "NH4+ donates its proton, so NH4+ acts as the acid.");
assert(result.correct && result.complete && !result.masteryAwarded);
vocabulary = new E.VocabularySession(acid);
result = vocabulary.submit("I do not know", "");
assert(result.taught && vocabulary.state.supported && vocabulary.state.retrievalPending);
vocabulary.beginTeachBack();
result = vocabulary.submit("It donates a proton", "NH4+ is an acid because it donates H+.");
...
for (const day of [4,5,6,7,8,9]) {
  const first = E.supportVisual(day, false), changed = E.supportVisual(day, true);
  assert(first.includes("<svg") && first.includes(`data-day=\"${day}\"`));
  assert.notEqual(first, changed);
}
assert(E.supportVisual(7, false).includes("source → destination"));
assert(E.supportVisual(7, false).includes("C—X bond arrow → X⁻"));
assert(E.supportVisual(8, false).includes("transition-state peak (ΔG‡)"));
assert(E.supportVisual(8, false).includes("products (ΔG)"));
