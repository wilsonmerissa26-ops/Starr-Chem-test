...
check("phone release observer guards against a detached browser document",/function hasDocument\(\)/.test(phoneFix)&&/if\(!hasDocument\(\)\)return/.test(phoneFix));
check("phone release observer disconnects during page teardown",/pagehide/.test(phoneFix)&&/observer\.disconnect\(\)/.test(phoneFix));
